data = """bot 152 gives low to bot 155 and high to bot 70
...
bot 94 gives low to bot 23 and high to bot 19""".split("\n")
...
def hook_up_bots(line, botnet):
    ...
    botnet[subject]['lowbot'] = low
    botnet[subject]['highbot'] = high
    if "output" in low:
        botnet[low] = {'chips': []}
    if "output" in high:
        botnet[high] = {'chips': []}
...
def activate_botnet(value, subject, botnet):
...
def execute_instructions(data):
    ...
    for line in data:
        if line.startswith("bot"):
            hook_up_bots(line, botnet)
    for line in data:
        if line.startswith("value"):
            value, subject = get_value_args(line)
            activate_botnet(value, subject, botnet)
    return botnet
