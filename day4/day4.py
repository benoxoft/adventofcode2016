...
bxaxipgn-vgpst-qphzti-rdcipxcbtci-635[ipctx]""".split("\n")
...
def find_sum_of_sectors(data):
    sum_of_sectors = 0
    for line in data:
        tokens = line.split("-")
        encrypts = ''.join(tokens[:-1])
        sector = int(tokens[-1][:-7])
        checksum = tokens[-1][-6:-1]
        position = 0
        track = []

        track = {}
        for letter in encrypts:
            track[letter] = encrypts.count(letter)

        top5 = sorted(track.keys(), key=lambda x: track[x])[-5:]

        decoy = False
        for letter in checksum:
            if not letter in track.keys():
                decoy = True
                break
        if decoy:
            continue
        for i in range(0, 5):
            ...
            if letter in top5:
                continue
            elif i != 0 and track[letter] == track[checksum[i-1]] and ord(checksum[i-1]) < ord(letter):
                continue
            elif i != 4 and track[letter] == track[checksum[i+1]] and ord(letter) < ord(checksum[i+1]):
                continue
            elif i == 4 and track[letter] == track[top5[0]]:
                continue
            else:
                decoy = True
                break
        ...
        if decoy:
            print(encrypts, " - ", checksum)
        else:
            sum_of_sectors += sector

example2 = "qzmt-zixmtkozy-ivhz-343[asdfg]"
def decrypt_line(line):
    splitted_line = [c for c in line]
    tokens = line.split("-")
    sector = int(tokens[-1][:-7])
    for i in range(0, sector):
        ...
        for j in range(0, top):
            if splitted_line[j] == '-':
                continue
            elif splitted_line[j] == 'z':
                splitted_line[j] = 'a'
            else:
                splitted_line[j] = chr(ord(splitted_line[j])+1)
    return ''.join(splitted_line)
#print(decrypt_line(example2))
#print(find_sum_of_sectors(data))
for line in data:
    line = decrypt_line(line)
    if 'north' in line:
        print(line)
