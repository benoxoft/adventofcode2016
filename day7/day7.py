data = """wysextplwqpvipxdv[srzvtwbfzqtspxnethm]syqbzgtboxxzpwr[kljvjjkjyojzrstfgrw]obdhcczonzvbfby[svotajtpttohxsh]cooktbyumlpxostt
...
def has_ABBA(token: str) -> bool:
    for i in range(0, len(token) - 3):
        if token[i] == token[i+3] and token[i+1] == token[i+2] and token[i] != token[i+2] and token[i+1] != token[i+3]:
            return True
    return False


def find_ABA(token: str) -> list:
    abas = list()
    for i in range(0, len(token) - 2):
        if token[i] == token[i+2] and token[i] != token[i+1]:
            abas.append(token[i:i+3])
    return abas


def has_BAB(token: str, abas: list) -> bool:
    for i in range(0, len(token) - 2):
        if token[i] == token[i + 2] and token[i] != token[i + 1]:
            aba = token[i+1] + token[i] + token[i+1]
            if aba in abas:
                return True
    return False


def support_TLS(line: str) -> bool:
    supernets, hypernets = split_line(line)
